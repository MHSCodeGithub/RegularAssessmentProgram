            ...
            u.save()
              .then(newUser => {
                ...
                if (itemsProcessed == array.length) {
                  if (schoolCount > 0) {
                    if (schoolTotal > 0) {
                      currentPeriod.average = Number(
                        schoolTotal / schoolCount
                      ).toFixed(2);
                      //console.log("Whole School: " + currentPeriod.average);
                    }
                    if (year7total > 0) {
                      currentPeriod.year7 = Number(
                        year7total / year7count
                      ).toFixed(2);
                      //console.log("Year 7: " + currentPeriod.year7);
                    }
                    if (year8total > 0) {
                      currentPeriod.year8 = Number(
                        year8total / year8count
                      ).toFixed(2);
                      //console.log("Year 8: " + currentPeriod.year8);
                    }
                    if (year9total > 0) {
                      currentPeriod.year9 = Number(
                        year9total / year9count
                      ).toFixed(2);
                      //console.log("Year 9: " + currentPeriod.year9);
                    }
                    if (year10total > 0) {
                      currentPeriod.year10 = Number(
                        year10total / year10count
                      ).toFixed(2);
      ...
      students.forEach(function(student) {
        ...
        for (var i = 1; i < student.rap.length; i++) {
          if (student.rap[i].average > 0 && student.rap[i - 1].average > 0) {
            student.rap[i].change = Number(
              student.rap[i].average - student.rap[i - 1].average
            ).toFixed(2);
          } else {
            ...
            u.rap.forEach(function(r) {
              if (
                ...
                if (r.grade == 7 && r.average >= 3) {
                  ...
                  year10 +=
                    u.name + " (" + Number(r.average).toFixed(2) + ")\n";
                  count10++;
                }
              }
            });
  ...
  RapPeriods.find({}).then(function(periods) {
    ...
    periods.forEach(function(p) {
      if (p.year == year && p.term == term && p.week == week) {
        ...
        p.save().then(period => {
          console.log(
            "Current RAP Period is now current: " +
              p.year +
              ", " +
              p.term +
              ", " +
...
router.get("/getRapPeriods", (req, res) => {
  RapPeriods.find({})
    .sort({ year: "descending", term: "descending", week: "descending" })
...
router.get("/teacher", (req, res) => {
  ...
  RapPeriods.findOne({ $or: [{ active: true }, { current: true }] })
    .sort({ active: -1 })
                    ...
                    classes.forEach(function(c) {
                      ...
                      if (c.code == s.code) {
                        ...
                        c.students.push({
                          ...
                          score: s.value,
                          id: u.id
                        }); // if class exists push the student into the array
                      }
                    });
                    if (!classFound) {
                      // if class NOT found then push the class and student into the array
                      let studentsArr = [
                        { name: u.name, score: s.value, id: u.id }
                      ];
                      classes.push({
                        code: s.code,
                        subject: s.subject,
...
router.get("/student", (req, res) => {
  ...
  Student.findOne({ name: req.query.name }).then(function(stu) {
    if (req.session.user.name == req.query.name) {
      ...
      console.log(
        req.session.user.name + " looked up scores for " + req.query.name
      );
    }
...
router.post("/addTeacher", (req, res) => {
  ...
  Teacher.findOne({ name: teacher }, function(err, user) {
    ...
    if (user) {
      ...
      tch.save().then(stu => {
        console.log(
          req.session.user.name + " successfuly added " + teacher + " to list"
        );
        res.send(JSON.stringify(true));
      });
    }
  ...
  Student.find({}).then(function(users) {
    ...
    users.forEach(function(u, index, array) {
      Student.findOne({ name: u.name }, function(err, stu) {
        if (err) {
          console.log("Something went wrong when searching the data!");
        }
        stu.rap.forEach(function(r) {
...
router.get("/autocompleteClasses", (req, res) => {
  RapPeriods.findOne({ $or: [{ current: true }, { active: true }] }, function(
    err,
    currentPeriod
  ) {
    Student.find({
          ...
          if (err) {
            ...
            for (var i = 0; i < teacherArray.length; i++) {
              for (var j = 0; j < teacherArray[i].length; j++) {
                if (
                  teacherArray[i][j].scores.reduce(function(acc, val) {
                    return acc + val;
                  }) == 0
                ) {
                  classesNotDone.push({
                    code: teacherArray[i][j].code,
                    teacher: teacherArray[i][j].teacher
                  });
                }
              }
            }
...
router.get("/countScores", (req, res) => {
  RapPeriods.findOne({ current: true }).then(function(currentPeriod) {
    ...
    Student.find({}).then(function(students) {
      ...
      averages.whole = Number(
        (whole[0] + whole[1] * 2 + whole[2] * 3 + whole[3] * 4 + whole[4] * 5) /
          totals.whole
      ).toFixed("2");
      averages.year7 = Number(
        (year7[0] + year7[1] * 2 + year7[2] * 3 + year7[3] * 4 + year7[4] * 5) /
          totals.year7
      ).toFixed("2");
      averages.year8 = Number(
        (year8[0] + year8[1] * 2 + year8[2] * 3 + year8[3] * 4 + year8[4] * 5) /
          totals.year8
      ).toFixed("2");
      averages.year9 = Number(
        (year9[0] + year9[1] * 2 + year9[2] * 3 + year9[3] * 4 + year9[4] * 5) /
          totals.year9
      ).toFixed("2");
      averages.year10 = Number(
        (year10[0] +
          year10[1] * 2 +
          year10[2] * 3 +
          year10[3] * 4 +
...
router.get("/getWholeAverage", (req, res) => {
  ...
  if (current == "true") {
    ...
    RapPeriods.find({})
      .sort({ year: "ascending", term: "ascending", week: "ascending" })
      .then(function(allPeriods) {
        var averages = [];
        var periods = [];
      ...
      Student.find({}).then(function(students) {
        allPeriods.forEach(function(currentPeriod) {
          ...
          if (maleTotal > 0 && maleCount > 0) {
            ...
            data.female.averages.push(
              Number(femaleTotal / femaleCount).toFixed("2")
            );
          }
        });
      ...
      async.eachSeries(
          ...
          if (
            ...
            teacher =
              ...
              teacher
                ...
                .slice(1)
                .toLowerCase();
            teacher = teacher.replace(/(^|[\s-])\S/g, function(match) {
            ...
            let firstname =
              student["First name"].charAt(0).toUpperCase() +
              student["First name"].slice(1).toLowerCase();
            let studentName = firstname + " " + surname;
            studentName = studentName.replace(/(^|[\s-])\S/g, function(match) {
              return match.toUpperCase();
            }); // capitalize after hyphen
            ...
            Student.findOne({ id: idNum }, function(err, user) {
              ...
              if (user) {
                ...
                if (!found) {
                  ...
                  user.rap[user.rap.length - 1].scores.push({
                    subject: student["Subject"],
                    code: student["Course code"] + student["Class id"],
                    teacher: teacher,
                    value: 0
                  });
                  user.save().then(newUser => {
                  ...
                  console.log(
                    "RAP period " +
                      user.rap[user.rap.length - 1].year +
                      ", term " +
                      user.rap[user.rap.length - 1].term +
                      ", week " +
                      user.rap[user.rap.length - 1].week +
                      " already exsists for " +
                      studentName
                  );
                  ...
                  async.eachSeries(
                    user.rap[user.rap.length - 1].scores,
                    function(score, callback2) {
        ...
        function(err) {
          if (err) {
                    ...
                    try {
                      ...
                      if (!foundTeacher) {
                        ...
                        try {
                          ...
                          console.log(
                            "Added " + s.teacher + " to the list of teachers"
                          );
                        } catch (err) {
            ...
            req.flash(
              "success_msg",
              "All students have been imported successfully"
            );
      ...
      async.eachSeries(
          ...
          if (student["studentNo"] == null || student["DEC User ID"] == null) {
            ...
            Student.findOne({ id: id }, function(err, user) {
              ...
              if (user) {
                ...
                user.save().then(stu => {
                  console.log(
                    "Username '" + stu.username + "' updated for " + stu.name
                  );
                });
                callback();
              } else {
        ...
        function(err) {
          if (err) {
            ...
            req.flash(
              "success_msg",
              "All student usernames been updated successfully"
            );
            res.redirect("/importEMU");
            return null;
          }
    ...
    async.eachSeries(
      ...
      function(err) {
        if (err) {
          ...
          req.flash(
            "success_msg",
            "All teachers have been imported successfully"
          );
                ...
                async.eachSeries(
                  ...
                  function(err) {
                    if (err) {
                      ...
                      console.log(
                        "All RAP Data has been imported successfully"
                      );
                      fs.unlink(jsonFilePath, err => {
                        if (err) {
                          console.log("An error occured");
              ...
              if (user) {
                ...
                for (let i = 0; i < user.rap.length; i++) {
                  if (
                    ...
                    user.save().then(newUser => {
                      console.log(
                        "Updating RAP period data for: " + studentName
                      );
                      callback();
                    });
                  }
                ...
                if (!found) {
                  ...
                  user.save().then(newUser => {
                    console.log(
                      "Adding old RAP period data for: " + studentName
                    );
                    callback();
                  });
                }
      ...
      Student.find({
        ...
        if (count > 0) {
          ...
          currentPeriod.save().then(newPeriod => {
            ...
            console.log(
              "Year 7: " + Number(year7total / year7count).toFixed(2)
            );
            console.log(
              "Year 8: " + Number(year8total / year8count).toFixed(2)
            );
            console.log(
              "Year 9: " + Number(year9total / year9count).toFixed(2)
            );
            console.log(
              "Year 10: " + Number(year10total / year10count).toFixed(2)
            );
          });
        }
      });
      ...
      async.eachSeries(
          ...
          if (student["StudentCode"] == null || student["M/F"] == null) {
            ...
            Student.findOne({ id: id }, function(err, user) {
              ...
              if (user) {
                ...
                user.save().then(stu => {
                  console.log(
                    "Gender '" + stu.gender + "' updated for " + stu.name
                  );
                });
                callback();
              } else {
        ...
        function(err) {
          if (err) {
            ...
            req.flash(
              "success_msg",
              "All student genders have been updated successfully"
            );
            res.redirect("/importLMBR");
            return null;
          }
